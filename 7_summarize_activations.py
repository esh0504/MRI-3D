...
def main():
    ...
    # ---- per-frame table ----
    with open(os.path.join(out, "7_activations_per_frame.csv"), "w") as f:
        f.write("frame,time," + ",".join(names) + "\n")
        for i in range(T):
            f.write(f"{i+1},{t[i]:.4f}," + ",".join(f"{v:.5f}" for v in A[i]) + "\n")
    print(f"[out] 7_activations_per_frame.csv  ({T} frames x {M} muscles)")
    ...
    if segs:
        with open(os.path.join(out, "7_activations_per_interval.csv"), "w") as f:
        ...
        print(f"[out] 7_activations_per_interval.csv  ({len(segs)} intervals)")
    ...
    fig.savefig(os.path.join(out, "7_activations_heatmap.png"), dpi=130, bbox_inches="tight")
    plt.close(fig); print("[out] 7_activations_heatmap.png")
    ...
    fig.savefig(os.path.join(out, "7_activations_peaks.png"), dpi=130, bbox_inches="tight")
    plt.close(fig); print("[out] 7_activations_peaks.png")
